 ...
 */
export async function startApp(offline = false): Promise<StartAppResponse> {
...
export function findLatestBuild(): string {
  ...
  .map((fileName) => {
    ...
    return undefined; // Explicitly return undefined for non-builds
  })
  .filter((file): file is { name: string; time: number } => file !== undefined) // Filter out undefined values
...
export function parseElectronApp(buildDir: string): ElectronAppInfo {
  ...
  let arch: Architecture | undefined = undefined;
  ...
  if (platform === 'darwin') {
    ...
    const appDir = path.join(buildDir, appBundle as string, 'Contents', 'MacOS');
    const appName = fs.readdirSync(appDir)[0];
    executable = path.join(appDir, appName);

    resourcesDir = path.join(buildDir, appBundle as string, 'Contents', 'Resources');
    ...
    executable = path.join(buildDir, exe as string);
