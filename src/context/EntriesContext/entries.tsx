      ...
      entries.map(async (entry: Entry) => {
        ...
        if (isArrayEmpty(data)) {
          ...
          if (!entryOnServer[0]) {
            ...
            const created_at_same = dayjs(entryOnServer[0].created_at).isSame(entry.created_at);
            const revisions_missmatch = entryOnServer[0].revision != entry.revision;
...
const syncEntries = async ({
  ...
  try {
    ...
    const newDaysToFetch: Day[] = [];
    const entriesToUpdateQueue: Day[] = [];
    ...
    if (!initialEntriesFetchDone.current) {
      ...
      const { data: daysSupabase, error } = await supabase
        .from<Entry>('journals')
        .select('day, revision')
      ...
      if (newDaysToFetch.length) {
        ...
        const numberOfChunks = Math.ceil(newDaysToFetch.length / daysInChunk);
        ...
        await Promise.all(
          chunks.map(async (v, i) => {
            ...
            const cursor = i * daysInChunk;
            const chunk = newDaysToFetch.slice(cursor, cursor + daysInChunk);

            const orString = chunk.map((s) => `day.eq.${s}`).join();
            logger('orString:');
            logger(orString);
            const { data: additionalDaysSupabase, error: error2 } = await supabase
              .from<Entry>('journals')
              .select()
              .eq('user_id', session.user.id)
              .or(orString);
